...
<body>
	...
	<c:url var="addAction" value="/spring/users/create/"></c:url>
		...
		<table>
			...
			<tr>
				<td colspan="2"><c:if test="${!empty user.id}">
						...
						<input type="submit" value="<spring:message text="Neu"/>" />
					</c:if></td>
			</tr>
		</table>
	</form:form>
		...
		<table class="tg">
			...
			<c:forEach items="${users}" var="user">
				<tr>
					...
					<td>${user.email}</td>
					<td>${user.active}</td>
					<td><a href="<c:url value='/spring/users/show/${user.id}' />">Show</a></td>
					<td><a
						href="<c:url value='/spring/users/delete/${user.id}' />">Delete</a></td>
				</tr>
			</c:forEach>
		</table>
	</c:if>
